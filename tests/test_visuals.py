...
def test_live_history_replays_complete_and_failed_episodes(browser: Browser, workbench_url: str):
    ...
    expect(page.locator("#check-answer .check-answer-value")).to_have_text("Yes")
    expect(page.locator("#check-answer .check-answer-detail")).to_have_text("News changed the result.")
...
def test_context_is_plain_language_and_onset_safe(page: Page):
    ...
    value_tops = page.locator("#signal-grid .signal strong").evaluate_all(
        "values => values.map(value => Math.round(value.getBoundingClientRect().top))"
    )
    assert len(set(value_tops)) == 1
...
def test_selected_anomaly_has_inline_explanation_check(page: Page):
    ...
    page.get_by_role("button", name="Previous").click()
    expect(page.locator("#episode-title")).to_have_text("LUNAUSDT")
    expect(page.locator("#check-answer .check-answer-value")).to_have_text("No")
    expect(page.locator("#check-answer .check-answer-detail")).to_have_text("Result stayed the same.")
    expect(page.locator("#check-answer .check-answer-value")).to_have_css("font-size", "28px")
    expect(page.locator("#check-answer .check-answer-detail")).to_have_css("font-size", "14px")
