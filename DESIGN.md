...
- **Deployment boundary:** GitHub Pages hosts historical replay only. Public live observation is served with same-origin API from one Raspberry Pi application container through Tailscale Funnel HTTPS. PostgreSQL and news services remain private. Interrupted pending analyses become visible failures rather than automatic paid retries.
